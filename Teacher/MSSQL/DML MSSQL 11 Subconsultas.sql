...
select a.almacen_nombre, a.almacen_direccion, a.codigo_postal, a.ciudad, v.total
from almacenes as a
inner join (
...
la cantidad de empleados que tienen asignados ese cargo. 
...
*/
SELECT cargo_descripcion, nivel_maximo, nivel_minimo, COUNT(e.cargo_id) AS cantidad
FROM empleados as e
right JOIN cargos as c on c.cargo_id = e.cargo_id
GROUP BY c.cargo_id, cargo_descripcion, nivel_maximo, nivel_minimo

SELECT t.cargo_descripcion, t.nivel_maximo, t.nivel_minimo,
	isnull(te.cantidad,0) AS cantidad
FROM cargos AS t
LEFT JOIN (
	SELECT cargo_id, COUNT(cargo_id) AS cantidad
	FROM empleados
	GROUP BY cargo_id
...
/* Informar nombre y apellido de los empleados junto a su nivel minimo de cargo */
select nombre, apellido
	, (
		select nivel_maximo / 2 + nivel_minimo / 2
		from cargos
